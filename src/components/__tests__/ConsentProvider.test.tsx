...
const Probe = ({ onChange }: { onChange?: (s: any) => void }) => {
  ...
  useEffect(() => {
    onChange?.(ctx);
  }, [ctx.hasConsented, ctx.preferences.marketing]);
...
describe("ConsentProvider (cookie consent + ad script gating)", () => {
  beforeEach(() => {
    ...
    document
      .querySelectorAll('script[id^="heartsync-"]')
  ...
  it("re-injects the Monetag zone tag when server settings arrive after mount (reload race)", () => {
    // Regression: a returning visitor has stored consent, so the mount effect
    // injects ads immediately, but on a reload it runs BEFORE /api/state
    // resolves, monetag_active is still false, and the zone tag was silently
    // skipped forever. The provider must retry once settings land.
    ...
    expect(document.getElementById("heartsync-monetag-script")).toBeNull(); // settings not loaded yet
    act(() => {
      // loadServerState resolves: server settings land and listeners fire
      heartsync.site_settings.monetag_active = true;
      heartsync.site_settings.monetag_zone_id = "284167";
      heartsync.triggerUpdate();
    });
    const tag = document.getElementById("heartsync-monetag-script");
    expect(tag).not.toBeNull();
    expect(tag?.getAttribute("src")).toBe(
      "https://alwingulla.com/284167/tag.min.js",
    );
    expect(tag?.getAttribute("data-zone")).toBe("284167");
  });
  ...
  it("starts in default-DENIED consent mode when the visitor has not chosen (Consent Mode v2)", () => {
    ...
    const consentDefaults = (window.dataLayer || []).find(
      (e: any[]) => e[0] === "consent" && e[1] === "default",
    );
  ...
  it("acceptAll persists consent + full preferences and grants consent mode", () => {
    ...
    render(
      <ConsentProvider>
        <Probe onChange={(c) => (ctx = c)} />
      </ConsentProvider>,
    );
    act(() => ctx.acceptAll());
    expect(localStorage.getItem("heartsync_cookie_consent")).toBe(
      JSON.stringify("accepted"),
    );
    expect(
      JSON.parse(localStorage.getItem("heartsync_cookie_preferences")!),
    ).toEqual({
      necessary: true,
      analytics: true,
      marketing: true,
    ...
    const update = [...all]
      .reverse()
      .find((e: any[]) => e[0] === "consent" && e[1] === "update");
  ...
  it("rejectAll records rejection and NEVER injects Monetag/Adsterra/Meta scripts", () => {
    ...
    act(() => ctx.rejectAll());
    expect(JSON.parse(localStorage.getItem("heartsync_cookie_consent")!)).toBe(
      "rejected",
    );
  ...
  it("injects Monetag only when active + zone configured + marketing consented, never with fake defaults", () => {
    ...
    expect(s!.getAttribute("src")).toBe(
      "https://alwingulla.com/987654/tag.min.js",
    );
  });
