...
const supabase = vi.hoisted(() => {
  ...
  function makeChain(table: string, clientToken: string | null) {
    ...
    const rowOutcome = () => {
      ...
      if (table === "profiles" && clientToken) {
        ...
        return p
          ? { data: p, error: null }
          : { data: null, error: { message: "no profile row" } };
      }
    ...
    chain.then = (resolve: any, reject: any) =>
      Promise.resolve(kind.write ? writeOutcome() : listOutcome()).then(
        resolve,
        reject,
      );
    chain.catch = (cb: any) =>
  ...
  const createClient = (_url: any, _key: any, opts?: any) => {
    const clientToken =
      (opts?.global?.headers?.Authorization || "").split(" ")[1] || null;
    return {
      auth: {
      ...
      rpc: (name: string, _args?: any) => {
        ...
        return Promise.resolve(
          name === "digital_product_download"
            ? state.rpcOutcome
              ? { data: state.rpcOutcome, error: null }
...
const storeSrc = fs.readFileSync(
  path.resolve(__dirname, "../../src/store.ts"),
  "utf8",
);
...
beforeAll(async () => {
  ...
  supabase.state.users["admin-token"] = {
    id: "u-admin",
    email: "admin@heartsync.app",
  };
  supabase.state.profiles["admin-token"] = {
    role: "admin",
    is_suspended: false,
  };
  supabase.state.users["reader-token"] = {
    id: "u-reader",
    email: "reader@heartsync.app",
  };
  supabase.state.profiles["reader-token"] = {
    role: "user",
    is_suspended: false,
  };
  supabase.state.users["editor-token"] = {
    id: "u-editor",
    email: "editor@heartsync.app",
  };
  supabase.state.profiles["editor-token"] = {
    role: "Editor",
    is_suspended: false,
  };
  supabase.state.users["suspended-token"] = {
    id: "u-sus",
    email: "sus@heartsync.app",
  };
  supabase.state.profiles["suspended-token"] = {
    role: "admin",
    is_suspended: true,
  };

  supabase.state.tables["site_settings"] = [
    { id: "singleton", site_name: "Heartsync" },
  ];
...
describe("auth matrix (live app, mocked Supabase)", () => {
  ...
  it.each(PROTECTED_ROUTES)(
    "%s %s admits an admin session (no 401/403)",
    async (method, route) => {
      const res = await req(
        method.toUpperCase(),
        route,
...
describe("admin flows", () => {
  ...
  it("integrations save surfaces upsert errors instead of faking success", async () => {
    supabase.state.insertErrByTable["integration_settings"] = {
      message: "RLS violation",
    };
    const failRes = await req(
      "POST",
      "/api/admin/integrations/save",
      { integrationId: "payments", settings: { public_key: "pk_test_123" } },
      ADMIN,
    );
  ...
  it("setup wizard fails honestly without a service-role key", async () => {
    const res = await req("POST", "/api/setup/register", {
      email: "first.admin@heartsync.app",
      name: "First Admin",
...
describe("subscribe persistence", () => {
  it("persists a new subscriber and confirms", async () => {
    const res = await req("POST", "/api/subscribe", {
      email: "fresh.reader@example.com",
      source: "footer",
    });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("treats a UNIQUE-violation duplicate as idempotent success", async () => {
    supabase.state.insertErrByTable["subscribers"] = {
      code: "23505",
      message:
        'duplicate key value violates unique constraint "subscribers_email_key"',
    };
    const res = await req("POST", "/api/subscribe", {
...
describe("security headers", () => {
  it("applies the Content-Security-Policy to function-served responses", async () => {
    const res = await fetch(`${base}/api/state`);
    expect(res.ok).toBe(true);
    const csp = res.headers.get("content-security-policy") || "";
    // Ad serving contract: the ACTIVE providers' script domains MUST be in
    // script-src or the browser silently blocks every ad call (the exact bug
    // that made Monetag/Adsterra/AdSense invisible on article pages).
    expect(csp).toContain("script-src");
    expect(csp).toContain("https://alwingulla.com"); // Monetag MultiTag loader
    expect(csp).toContain("https://3nbf4.com"); // Monetag SW domain
    expect(csp).toContain("https://*.highperformanceformat.com"); // Adsterra invoke.js
    expect(csp).toContain("https://*.highrevenueformat.com"); // Adsterra banners
    expect(csp).toContain("https://*.profitableratecpmnetwork.com"); // Adsterra popunder
    expect(csp).toContain("https://pagead2.googlesyndication.com"); // AdSense loader
    expect(csp).toContain("https://tpc.googlesyndication.com"); // AdSense creatives
    expect(csp).toContain("https://*.doubleclick.net"); // AdSense auction
    expect(csp).toContain("https://connect.facebook.net"); // Meta pixel
  });
});
...
describe("source-level security contract", () => {
  ...
  it.each(PROTECTED_ROUTES)(
    "%s %s is gated by adminAuthMiddleware in source",
    (method, route) => {
      const sourceRoute = route
        .split("/")
        .map((seg) => (["x", "req-1", "prod-1"].includes(seg) ? ":id" : seg))
        .join("/");
      const m = serverSrc.match(
        new RegExp(
          `app\\.${method}\\('${esc(sourceRoute)}',\\s*adminAuthMiddleware`,
        ),
      );
  ...
  it("the reader-facing catalog, checkout, download, subscribe and DSR-create routes stay unauthenticated", () => {
    ...
    expect(serverSrc).toMatch(
      /app\.post\('\/api\/digital-products\/checkout',\s*async/,
    );
    expect(serverSrc).toMatch(
      /app\.get\('\/api\/digital-products\/download\/:token',\s*async/,
    );
    expect(serverSrc).toMatch(/app\.post\('\/api\/subscribe',\s*async/);
    expect(serverSrc).toMatch(
      /app\.post\('\/api\/gdpr\/dsr-requests',\s*rateLimiter/,
    );
  });
  ...
  it("both payment webhooks fulfil digital_product metadata through the service role", () => {
    expect(
      (serverSrc.match(/md\.kind === 'digital_product'/g) || []).length,
    ).toBe(2);
  ...
  it("client engagement goes through the SECURITY DEFINER RPC, never direct UPDATEs", () => {
    expect(
      (storeSrc.match(/rpc\('increment_post_engagement'/g) || []).length,
...
describe("schema contract", () => {
  ...
  it("is_admin() recognizes the full ADMIN_ROLES vocabulary", () => {
    expect(schemaSrc).toMatch(
      /IN \('admin', 'superadmin', 'Administrator', 'Editor', 'Super Admin'\)/,
    );
  });
  ...
  it("digital product tables, policies, and the token-download RPC exist", () => {
    ...
    expect(schemaSrc).toMatch(
      /CREATE POLICY "Public read active digital products"/,
    );
    expect(schemaSrc).toMatch(
      /CREATE OR REPLACE FUNCTION public\.digital_product_download/,
    );
  });

  it("subscribers keep a UNIQUE email constraint", () => {
    expect(schemaSrc).toMatch(
      /CREATE TABLE IF NOT EXISTS public\.subscribers[\s\S]*?email TEXT NOT NULL UNIQUE/,
    );
  });

  it("integration storage matches the key/value API model", () => {
    expect(schemaSrc).toMatch(
      /CREATE TABLE IF NOT EXISTS public\.integration_settings[\s\S]*?integration_id TEXT NOT NULL[\s\S]*?key TEXT NOT NULL/,
    );
  });

  it("legacy API keys are scrubbed from site_settings by the cleanup block", () => {
    expect(schemaSrc).toMatch(
      /SECURITY CLEANUP: API keys must NEVER live in site_settings/,
    );
    expect(schemaSrc).toMatch(
...
describe("GET /api/state comment PII boundary", () => {
  it(
    "never ships comment author_email in the public boot payload",
    { timeout: 30000 },
    async () => {
